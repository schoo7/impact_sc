...
main_try_catch_result <- tryCatch({
    ...
    cat("Installing magick...\\n")
    install.packages("magick", lib = user_lib_path, repos = "https://cloud.r-project.org/")
    cat("Installing jjAnno...\\n")
    install.packages("jjAnno", lib = user_lib_path, repos = "https://cloud.r-project.org/")
    ...
    # MODIFICATION: Explicitly install hdf5r before SeuratDisk and SeuratExtend
    cat("Explicitly installing hdf5r from CRAN before SeuratDisk...\n")
    install_if_missing("hdf5r", lib_path = user_lib_path)
