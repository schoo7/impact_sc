...
def main():
    ...
    rscript_exe_paths = ask_for_paths(
        prompt="Enter the full path to your Rscript executable (e.g., F:/R-4.4.2/bin/x64/Rscript.exe)",
...
def setup_custom_mode(downloaded_data: Dict[str, str], rscript_executable_path: str):
    ...
    scripts_dir_paths = ask_for_paths(
        prompt="Enter the full path to the directory containing R and Python module scripts",
        allow_multiple=False,
        is_optional=False,
        ensure_dir=True,
    ...
    input_data_paths_val = ask_for_paths(
        prompt="Enter the full path to your primary input scRNA-seq data file(s) (e.g., feature-barcode matrix directory, or an .RDS file like 'ori.RDS')",
        allow_multiple=True, 
        is_optional=False,
    ...
    if "02b_c2s" in params["selected_modules"]:
        ...
        h5ad_c2s_paths = ask_for_paths(
            prompt="Enter the full path to the H5AD file for Cell2Sentence input (e.g., F:/R_PROJECT/impact/02_module2_for_c2s.h5ad)",
            allow_multiple=False,
            is_optional=False, 
            ensure_file=True
        )
        ...
        c2s_model_input_list = ask_for_paths(
            prompt=f"Enter the Cell2Sentence model path (if local, e.g., F:/path/to/model_folder) or Hugging Face model name",
            allow_multiple=False,
            is_optional=False,
            ensure_dir=False, # Can be a name, not necessarily a dir
    ...
    if "04c_decoupler" in params["selected_modules"]:
        ...
        collectri_paths = ask_for_paths(
            prompt=collectri_prompt,
            allow_multiple=False,
            is_optional=False,
            ensure_file=True
        )
        ...
        if params["species"] == "human":
            ...
            # This call was already correct, using all positional args up to ensure_file properly
    ...
    if params["species"] in ["human", "mouse"]:
        ...
        # Use the more descriptive prompt from the user's latest full code for this part
        prompt_text_detail = f"Enter the full path to a local SingleR reference RDS file for {params['species']} (e.g., F:/R_PROJECT/impact/ref/{params['species']}ref.RDS)."
        if ref_suggestion:
            print(f"Tip: Downloaded reference data available: {ref_suggestion}")
            # If suggestion exists, the shorter prompt is fine, as suggestion will be appended by ask_for_paths
        else: # No suggestion, so use the more detailed prompt
            prompt_text = prompt_text_detail
        ...
        elif module_3_is_selected: # Mandatory if module 3 is selected
            if ref_suggestion and os.path.exists(ref_suggestion): 
                params["local_singler_ref_path"] = ref_suggestion
                print(f"Warning: No valid custom path provided for mandatory SingleR reference. Using suggested/downloaded reference: {ref_suggestion}")
            else:
                print(f"CRITICAL ERROR: Module 3 selected, but no local SingleR reference path obtained or valid suggested reference found.")
                sys.exit(1)
        else: 
            print("No local SingleR reference provided (optional or Module 3 not selected).")
    ...
    if "04f_query_projection" in params["selected_modules"]:
        ...
        query_rds_paths = ask_for_paths(
            prompt="Enter the full path to the query.RDS file (e.g., F:/R_PROJECT/impact/ref/query.RDS)",
            allow_multiple=False,
            is_optional=False,
            ensure_file=True
        )
