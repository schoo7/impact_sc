...
# Get paths from environment variables (these should be set by 02a_harmony_c2s_prep.R)
c2s_output_dir_full_path <- file.path(base_output_path, "C2S_output") # Reconstruct path based on base_output_path
c2s_embeddings_csv_path <- Sys.getenv("C2S_EMBEDDINGS_CSV", file.path(c2s_output_dir_full_path, "c2s_cell_embeddings.csv"))
c2s_predicted_csv_path <- Sys.getenv("C2S_PREDICTED_CSV", file.path(c2s_output_dir_full_path, "predicted_cell_types.csv"))
  ...
  if (!is.null(predicted_df_c2s) && nrow(predicted_df_c2s) > 0) {
    if ("text_ids" %in% colnames(predicted_df_c2s) && "predicted_label" %in% colnames(predicted_df_c2s)) {
        if (all(predicted_df_c2s$text_ids %in% colnames(obj))) {
            ...
            obj$c2s_direct_predicted_label <- c2s_preds_ordered 
