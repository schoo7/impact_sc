...
# Version: 2.0 (Conditional input loading based on annotation choice)
...
# --- MODIFIED SECTION: Conditional Input File Loading ---
# Determine which Seurat object to load. If 'c2s' is the chosen final annotation
# source, we must load the object that has had C2S results added to it.
# Otherwise, we load the standard harmony-processed object.

input_rds_path <- if (final_cell_type_source_choice == "c2s") {
  message("INFO: 'c2s' selected as final cell type source. Loading C2S-processed Seurat object.")
  file.path(base_output_path, "02_module2_c2s_processed.RDS")
} else {
  message("INFO: Loading standard Harmony-processed Seurat object.")
  file.path(base_output_path, "02_module2_harmony.RDS")
}

# Load the determined Seurat object
if (file.exists(input_rds_path)) {
  obj <- tryCatch(readRDS(input_rds_path), error = function(e) {
    message(paste("Error loading Seurat object from", input_rds_path, ":", e$message))
    return(NULL)
  })
  if (is.null(obj)) stop("Failed to load data for Module 3.")
} else {
  stop(paste("Required Seurat object not found at:", input_rds_path))
}
...
if (final_cell_type_source_choice == "seurat") {
  ...
  # Corrected to check for 'c2s_predicted_label'
  if ("c2s_predicted_label" %in% colnames(obj@meta.data)) {
    obj$celltype_final <- obj$c2s_predicted_label
    message("Final cell type set to 'c2s_predicted_label'.")
  } else {
    warning("'c2s' was chosen as final source, but 'c2s_predicted_label' column not found. This may be because the wrong input file was loaded. 'celltype_final' will be NA.")
  }
  ...
  } else if ("c2s_predicted_label" %in% colnames(obj@meta.data) && !all(is.na(obj$c2s_predicted_label))) {
    obj$celltype_final <- obj$c2s_predicted_label
    message("Auto-selected 'c2s_predicted_label' as final cell type.")
  } else {
    obj$celltype_final <- obj$seurat_clusters
